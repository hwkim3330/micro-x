...
기본형은 코 중앙 카메라 1개와 표정용 눈 2개를 사용합니다. Rev A에서는 카메라 벽이 머리 프레임에 통합되어 요·롤 서보와 함께 움직입니다. 두 카메라를 쓰는 스테레오 깊이 인식 버전은 보류 상태입니다. [버전별 설계 조건](../engineering/camera_variants.json)에 동기화, 보정, 깊이 오차, 연산 장치, 머리 질량 및 목 토크 검증을 명시했습니다. 렌즈 보호 테두리·배선 고정·내부 정비 구조는 설계 목표이며 낙하 강도가 검증됐다는 뜻은 아닙니다.
...
| 보행 | Rev A 15서보 구동 CAD, CAD 기반 동역학 모델, 공식 가중치 시뮬레이션 | 실물 조립·전원·제어·보행 |
...
- Rev A 로봇 좌표에서 카메라 벽 X45.5~47.5(머리 프레임 일체), 홀 Y±10.5, Z229.75/242.25, 렌즈 Ø12 포트 (50,0,236). 코 앞면 카메라 포트는 두개골 주둥이에 있습니다.
- 카메라 벽은 `head_frame`의 일부이며 네 M2 체결 위치를 가집니다. 나사 머리·너트·PCB 부품 간섭, FPC 경로(목 4축을 지나 몸통 보드까지), 조립 순서는 실물 확인이 필요합니다.
- 광축 Z236, 주둥이 앞면(X≈64)까지 약 16 mm 깊이의 Ø12 포트입니다. 66° 수평 시야에서 비네팅 여부는 실물 확인 대상이며, 필요하면 포트를 원뿔로 넓힙니다.
- FPC 홈을 두었지만 케이블 굽힘과 몸통까지의 배선은 미완성입니다. Pi Zero 계열에는 맞는 15→22핀 카메라 케이블이 필요합니다.

개발용 Pi 5 4GB와 제품 검토용 CM5 4GB를 우선 후보로 변경했습니다. [실행 위치·배선·검증 조건](ONBOARD_COMPUTE.md)을 참고하세요. 실제 보드에서 전체 비전·보행·신경망을 동시에 시험하기 전에는 확정 부품이 아닙니다. 배터리, 보호 회로, 모터 전원, 로직 전원과 발열 설계도 남아 있습니다.
