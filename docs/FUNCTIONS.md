...
기본형은 코 중앙 카메라 1개와 표정용 눈 2개를 사용합니다. 현재 P0의 단일 카메라 브래킷을 기준으로 하되, 14관절 기구로의 통합은 별도 검증이 필요합니다. 두 카메라를 쓰는 스테레오 깊이 인식 버전은 보류 상태입니다. [버전별 설계 조건](../engineering/camera_variants.json)에 동기화, 보정, 깊이 오차, 연산 장치, 머리 질량 및 목 토크 검증을 명시했습니다. 렌즈 보호 테두리·배선 고정·내부 정비 구조는 설계 목표이며 낙하 강도가 검증됐다는 뜻은 아닙니다.
