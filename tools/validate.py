"""Geometry and evidence consistency checks for Micro X Rev A. Runs with numpy + trimesh only."""
from pathlib import Path
import hashlib,json,re
import numpy as np
import trimesh
R=Path(__file__).resolve().parents[1];report=json.loads((R/'artifacts/parts.json').read_text());parts=report['parts'];checks=[]
assert len(parts)>=20 and all(p['printed'] for p in parts),'printed part list'
for p in parts:
...
out=dict(revision='RevA',printed_parts=len(parts),purchased_items=len(report['purchased']),mesh_checks=checks,printed_mass_g=report['printed_mass_g'],purchased_mass_g=report['purchased_mass_g'],
    geometry='valid generated CAD checked during build; closed positive-volume meshes checked here',interference='exact STEP intersections at HOME cleared; single-joint sampled sweeps recorded in artifacts/interference.json',
