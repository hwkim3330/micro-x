...
try{
  ...
  assert.equal((await api('/api/sim/start',{seed:0,policy:'../evil.onnx'})).status,400);checks++;
  assert.equal((await api('/api/evaluate',{checkpoint:'missing.onnx'})).status,400);checks++;
  const list=await(await fetch('http://127.0.0.1:5217/api/checkpoints')).json();assert.equal(list.official.file,'official');checks++;
  ...
  await page.goto('http://127.0.0.1:5217/web/simulator.html');await page.waitForFunction(()=>window.microXSimulation?.robot&&!document.querySelector('#sim-reset').disabled,{timeout:60000});
  assert.equal(await page.$eval('#sim-toggle',b=>b.disabled),true);checks++;
  await page.click('#sim-reset');await page.waitForFunction(()=>!document.querySelector('#sim-toggle').disabled,{timeout:60000});
  const initial=await page.evaluate(()=>microXSimulation.state);
  assert.equal(initial.qpos.length,7+15);assert.equal(initial.time,0);assert.equal(initial.policy,'official');checks+=3;
  ...
  await page.waitForFunction(()=>microXSimulation.state.time>=3,{timeout:60000});await page.keyboard.up('ArrowUp');await page.click('#sim-toggle');
  const moved=await page.evaluate(()=>microXSimulation.state);assert.ok(moved.position[0]>.005);assert.ok(moved.time>=3);checks+=2;
  // The rendered rig follows the physics: trunk node position equals the free-joint position.
  assert.ok(await page.evaluate(()=>Math.abs(microXSimulation.robot.bodies.trunk.position.x-microXSimulation.state.qpos[0])<1e-6));checks++;
  ...
  console.log(`${checks} live simulator checks passed: real time/position advance on the CAD rig, pause, reset, commands, sessions, origins, policy selection, desktop/mobile`);
