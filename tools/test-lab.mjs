...
try{
  ...
  for(const [name,width,height]of [['desktop',1440,1100],['mobile',390,844]]){
    const page=await browser.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.setViewport({width,height});await page.goto('http://127.0.0.1:5197/web/lab.html');await page.waitForFunction(()=>window.microXLab?.robot,{timeout:45000});
    ...
    const bad=path.join(directory,'foreign.json');await writeFile(bad,JSON.stringify({...downloaded,environment:'microduck'}));await(await page.$('#import')).uploadFile(bad);await page.waitForFunction(()=>document.querySelector('#training-status').textContent.includes('가상 턱 정책 파일만'));checks++;
  ...
  console.log(`${checks} lab browser checks passed: anatomy, training, replay, download/import, rejection, stop, mobile layout`);
